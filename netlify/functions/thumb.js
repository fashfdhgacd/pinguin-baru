const SVG = '<svg xmlns="http://www.w3.org/2000/svg" width="640" height="360"><rect width="640" height="360" fill="#161616"/><circle cx="320" cy="180" r="34" fill="#ff9000"/><polygon points="310,164 342,180 310,196" fill="#111"/></svg>';

exports.handler = async (event) => {
  const q = event.queryStringParameters || {};
  const id = String(q.id || "").replace(/[^A-Za-z0-9_-]/g, "");
  const h = String(q.h || "").toLowerCase();
  if (!id) return { statusCode: 200, headers: { "content-type": "image/svg+xml" }, body: SVG };
  const pages = [];
  if (h === "indoav") pages.push("https://tv1.indoav.app/e/" + id);
    ...
    pages.push("https://tv1.indoav.app/e/" + id);
    pages.push("https://tv1.userbokep.com/e/" + id);
  }
  for (const page of pages) {
    try {
      ...
      return {
        statusCode: 200,
        headers: { "content-type": ir.headers.get("content-type") || "image/jpeg", "cache-control": "public, max-age=604800" },
